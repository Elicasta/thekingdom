(function(global){
  var modules={'./QRMath': function(module, exports, require){
var QRMath = {
...
module.exports = QRPolynomial;
  ...
  var EC=req('./QRErrorCorrectLevel');
  function svg(text, options){
    options=options||{};
    var qr=new QRCode(-1, EC.M);
    qr.addData(String(text||''));
    qr.make();
    var count=qr.getModuleCount();
    var quiet=Number(options.quietZone||4);
    var total=count+quiet*2;
    var cells=[];
    for(var r=0;r<count;r++){
      for(var c=0;c<count;c++){
        if(qr.isDark(r,c)) cells.push('<rect x="'+(c+quiet)+'" y="'+(r+quiet)+'" width="1" height="1"/>');
      }
    }
    return '<svg class="join-qr-svg" viewBox="0 0 '+total+' '+total+'" role="img" aria-label="QR code to join the lesson" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="#fff"/><g fill="#081018">'+cells.join('')+'</g></svg>';
  }
  global.KingdomQR={svg:svg};
})(window);